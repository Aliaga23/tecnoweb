    ...
    <section class="section" style="background-color: var(--color-bg);">
      <div style="padding: 0 2rem;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
          <h1 class="section-title" style="margin-bottom: 0;">Gestión de Ventas al Contado</h1>
          <button @click="abrirModalCrearVenta" class="btn btn-primary">Nueva Venta al Contado</button>
        </div>

        <!-- Sección Ventas al Contado -->
            ...
            <tbody>
              ...
              <tr v-if="ventas.length === 0">
                <td colspan="8" style="padding: 2rem; text-align: center; color: var(--color-text-light);">
                  No hay ventas al contado registradas
                </td>
              </tr>
            </tbody>
            </table>
      ...
      <div class="card" style="width: 100%; max-width: 600px; max-height: fit-content; overflow: visible;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; border-bottom: 1px solid var(--color-border); padding-bottom: 0.75rem;">
          <h2 style="font-size: 1.25rem; font-weight: bold; color: var(--color-text); margin: 0;">
            Venta al Contado #{{ ventaSeleccionada?.id }}
          </h2>
...
<div v-if="modalPagoAbierto" style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background-color: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 10000; padding: 1rem;">
  <div class="card" style="width: 100%; max-width: 400px; max-height: fit-content; overflow: visible; padding: 1.25rem;">
      ...
      <div class="card" :style="pasoActual === 1 
        ...
        <div v-if="pasoActual === 2" style="margin-bottom: 1rem;">
          ...
          <div v-if="productosVenta.length > 0" style="background-color: var(--color-bg-alt); padding: 0.75rem; border-radius: 0.5rem; margin-top: 1rem;">
            <h4 style="color: var(--color-text); margin-bottom: 0.5rem; font-size: 0.95rem;">Método de Pago</h4>
            <div>
              <label style="display: block; margin-bottom: 0.25rem; color: var(--color-text); font-size: 0.875rem;">Seleccione método:</label>
              <select v-model="metodoPagoVenta" style="width: 100%; padding: 0.5rem; border: 1px solid var(--color-border); border-radius: 0.25rem; background-color: var(--color-bg); color: var(--color-text);">
            ...
            <p style="margin-top: 0.5rem; margin-bottom: 0; color: var(--color-text); font-size: 0.875rem; font-weight: 600;">
              Total a pagar: Bs. {{ totalVenta.toFixed(2) }}
            </p>
          </div>

          <div style="display: flex; justify-content: space-between; margin-top: 1rem;">
            <button @click="pasoActual = 1" class="btn btn-secondary">Cancelar</button>
            <button @click="crearVenta" class="btn btn-primary" :disabled="productosVenta.length === 0 || procesandoVenta || !metodoPagoVenta">
    ...
    <!-- Modal QR Venta Nueva -->
    <div v-if="modalQRVentaAbierto" style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background-color: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 10001; padding: 1rem;">
      <div class="card" style="width: 100%; max-width: 400px; max-height: fit-content; overflow: visible; padding: 1.25rem;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; border-bottom: 1px solid var(--color-border); padding-bottom: 0.75rem;">
          <h2 style="font-size: 1.125rem; font-weight: bold; color: var(--color-text); margin: 0;">
            Pago con QR - Venta al Contado
          </h2>
          <button @click="cerrarModalQRVenta" style="background: none; border: none; font-size: 1.5rem; cursor: pointer; color: var(--color-text); line-height: 1; padding: 0;">&times;</button>
        </div>

        <!-- Vista QR generando -->
        <div v-if="estadoQRVenta === 'generando'" style="text-align: center; padding: 2rem;">
          <div style="margin-bottom: 1rem; color: var(--color-text); font-size: 1.125rem;">Generando código QR...</div>
        </div>

        <!-- Vista QR para escanear -->
        <div v-else-if="estadoQRVenta === 'escaneando'" style="text-align: center;">
          <div style="margin-bottom: 1rem;">
            <img :src="ventaNuevaQRInfo?.qr_image" alt="Código QR" style="max-width: 280px; width: 100%; height: auto; border-radius: 0.5rem; border: 2px solid var(--color-border);">
          </div>
          <div style="background-color: var(--color-bg-alt); padding: 1rem; border-radius: 0.375rem; margin-bottom: 1rem;">
            <p style="margin: 0 0 0.5rem 0; color: var(--color-text); font-weight: 600;">Total a pagar:</p>
            <p style="margin: 0; color: #dc2626; font-size: 1.5rem; font-weight: bold;">Bs. {{ totalVenta.toFixed(2) }}</p>
          </div>
          <p style="color: var(--color-text); margin-bottom: 0.5rem; font-weight: 500;">Escanea el código QR con tu app de pagos</p>
          <p style="color: var(--color-text-light); font-size: 0.875rem;">Esperando confirmación del pago...</p>
          <div style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid var(--color-border);">
            <button @click="cerrarModalQRVenta" class="btn btn-secondary" style="padding: 0.625rem 1.25rem; font-size: 0.9375rem;">Cancelar</button>
          </div>
        </div>

        <!-- Vista pago completado -->
        <div v-else-if="estadoQRVenta === 'completado'" style="text-align: center; padding: 2rem;">
          <div style="color: #10b981; font-size: 3rem; margin-bottom: 1rem;">✓</div>
          <div style="color: var(--color-text); font-size: 1.25rem; font-weight: bold; margin-bottom: 0.5rem;">¡Pago completado!</div>
          <p style="color: var(--color-text-light); font-size: 0.9375rem; margin-bottom: 1.5rem;">La venta ha sido pagada exitosamente</p>
          <button @click="cerrarModalQRVentaCompletado" class="btn btn-primary" style="padding: 0.625rem 1.5rem; font-size: 0.9375rem;">Cerrar</button>
        </div>
      </div>
    </div>
...
// Estado del dashboard - Ventas al Contado
const ventas = ref([]);
...
const metodoPagoVenta = ref('');
const procesandoVenta = ref(false);

// Modal QR para venta nueva
const modalQRVentaAbierto = ref(false);
const ventaNuevaQRInfo = ref(null);
const estadoQRVenta = ref('generando');
const intervalVerificacionQRVenta = ref(null);
...
const registrarPago = async () => {
  ...
  // Si es pago con QR, generar código QR para contado
  if (metodoPago.value === 'qr') {
    await generarQRPagoContado();
    return;
  }
  ...
  try {
    const response = await apiFetch(`/api/ventas/${ventaSeleccionada.value.id}/pago-contado`, {
      method: 'POST',
      body: JSON.stringify({
...
const generarQRPagoContado = async () => {
  procesandoPago.value = true;
  ...
  try {
    ...
    const response = await apiFetch('/api/generar-qr-contado', {
      method: 'POST',
    ...
    if (data.success) {
      ...
      intervalVerificacionQR.value = setInterval(verificarEstadoQRContado, 5000);
    } else {
...
const verificarEstadoQRContado = async () => {
  try {
    ...
    console.error('Error al verificar estado QR contado:', error);
  }
};
...
const crearVenta = async () => {
  ...
  try {
    ...
    const response = await apiFetch('/api/ventas/contado', {
      method: 'POST',
      body: JSON.stringify(payload)
    });
    ...
    if (data.success) {
      // Si es efectivo o tarjeta, crear venta directamente
      if (metodoPagoVenta.value === 'efectivo' || metodoPagoVenta.value === 'tarjeta') {
        alert('Venta al contado creada y pagada exitosamente');
        cerrarModalCrear();
        await cargarVentas();
      } 
      // Si es QR, abrir modal para escanear
      else if (metodoPagoVenta.value === 'qr') {
        ventaNuevaQRInfo.value = data.data;
        await generarQRVentaNueva(data.data.id);
      }
    } else {
...
const registrarVisita = async () => {
  try {
    const response = await apiFetch('/api/visitas/dashboard-ventas-contado', {
      method: 'POST'
    });
    const data = await response.json();
    if (data.success) {
...
// Funciones para QR de venta nueva
const generarQRVentaNueva = async (ventaId) => {
  modalQRVentaAbierto.value = true;
  estadoQRVenta.value = 'generando';

  try {
    const response = await apiFetch('/api/generar-qr-venta-contado', {
      method: 'POST',
      body: JSON.stringify({
        venta_id: ventaId
      })
    ...
    if (data.success) {
      ventaNuevaQRInfo.value = data;
      estadoQRVenta.value = 'escaneando';
      
      // Iniciar verificación automática cada 5 segundos
      intervalVerificacionQRVenta.value = setInterval(verificarEstadoQRVentaNueva, 5000);
    } else {
      alert('Error al generar QR: ' + (data.error || 'Error desconocido'));
      cerrarModalQRVenta();
    }
...
const verificarEstadoQRVentaNueva = async () => {
  try {
    if (!ventaNuevaQRInfo.value?.pago_id) return;

    const response = await apiFetch(`/api/verificar-pago/${ventaNuevaQRInfo.value.pago_id}`);
    const data = await response.json();

    if (data.pagado) {
      estadoQRVenta.value = 'completado';
      if (intervalVerificacionQRVenta.value) {
        clearInterval(intervalVerificacionQRVenta.value);
        intervalVerificacionQRVenta.value = null;
      }
    }
  } catch (error) {
    console.error('Error al verificar estado QR venta:', error);
  }
};

const cerrarModalQRVenta = () => {
  modalQRVentaAbierto.value = false;
  ventaNuevaQRInfo.value = null;
  estadoQRVenta.value = 'generando';
  if (intervalVerificacionQRVenta.value) {
    clearInterval(intervalVerificacionQRVenta.value);
    intervalVerificacionQRVenta.value = null;
...
const cerrarModalQRVentaCompletado = async () => {
  cerrarModalQRVenta();
  cerrarModalCrear();
  await cargarVentas();
};
